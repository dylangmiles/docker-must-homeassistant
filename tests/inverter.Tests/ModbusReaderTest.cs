        ...
        public void Read_Success()
        {
            ...
            port.Setup(foo => foo.Read(It.IsAny<byte[]>(), It.IsAny<int>(), It.IsAny<int>())).Callback<byte[], int, int>((b,o,c) => {
                ...
                using( var writer = new BigEndianBinaryWriter(stream))
                {
                    ...
                    writer.Write((byte)0xCC);   //Check sum
                    writer.Write((byte)0xDD);   //Check sum
                }
            ...
            CollectionAssert.AreEqual(new byte[] { 0x01, 0x03, 0x07, 0xD0, 0x00, 0x05, 0xCC, 0xDD }, request);
            CollectionAssert.AreEqual(new byte[] { 0x01, 0x03, 0x0A, 0x00, 0xA, 0x00, 0x0B, 0x00, 0x0C, 0x00,0x0D,0x00,0x0E,0xCC,0xDD }, response);
